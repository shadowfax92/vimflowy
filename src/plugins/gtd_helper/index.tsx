...
// TODO: move all from _getChildren to getChildren which is based on path rather than row.
class GtdHelperPlugin {
    ...
    private async processGtdAction(session: Session) {
        ...
        // optimze by listening to row delete handler.
        ...
        await Promise.all(gtdTopicNodes.map(async (topicNodeRow) => {
            ...
            gtdTaskNodes.forEach(async (child_row) => {
                ...
                if (child_text.toLowerCase().indexOf(topic) === -1) {
                    ...
                    this.log('Deleting Node: ' + child_row + ' as it\'s gtd topic doesn\'t match ' + topic);
    ...
    private async cloneGtdRows() {
        ...
        for (let key in this.gtdContainedRows) {
            ...
            let it = gtdrows.values();
            for (let val = null; val = it.next().value;) {
                if (this.clonedRows.has(val)) {
...
registerPlugin(
    {
        ...
        description: (
            <div>
                Lets you:
      <ul>
                    <li> Track and extract GTD tagged items automatically. </li>
                </ul>
            </div>
        ),
    },
